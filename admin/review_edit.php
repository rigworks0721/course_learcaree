...
$stmt = $pdo->prepare('SELECT * FROM lp_reviews WHERE id = ? LIMIT 1');
$stmt->execute([$id]);
$review = $stmt->fetch();

if (!$review) {
    $_SESSION['flash_message'] = '対象レビューが見つかりません。';
    header('Location: review_dashboard.php');
    exit;
}
...
$errors = [];
$formData = [
    'posted_date' => (string)($review['posted_date'] ?? ''),
    'age_group' => (string)$review['age_group'],
    'gender' => (string)$review['gender'],
    'rating' => (int)$review['rating'],
    'comment' => (string)$review['comment'],
    'target_page' => (string)$review['target_page'],
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        try {
            $updateStmt = $pdo->prepare('UPDATE lp_reviews SET posted_date = ?, age_group = ?, gender = ?, rating = ?, comment = ?, target_page = ?, updated_at = NOW() WHERE id = ?');
            $updateStmt->execute([
            ...
            $_SESSION['flash_message'] = 'レビューを更新しました。';
            header('Location: review_dashboard.php');
            exit;
        } catch (Throwable $e) {
            $errors[] = 'レビューの更新に失敗しました。';
        }
    }
}
?>
  ...
  <div class="admin-layout">
    ...
    <main class="main">
      <header class="page-head">
        <h2 class="page-title">レビュー編集（ID: <?= $id ?>）</h2>
      </header>
        ...
        <form method="post">
          ...
          <div class="form-grid">
            ...
            <div class="field">
              ...
              <input id="target_page" name="target_page" type="text" value="<?= htmlspecialchars($formData['target_page'], ENT_QUOTES, 'UTF-8') ?>" required>
            </div>
